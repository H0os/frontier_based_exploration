...
class MapUtils(Node):
    ...
    def find_frontiers(self, grid_data, resolution, origin):
        """
        Finds all frontier centroids using the Naïve Active Area method.
        ...
        if not self.tree_map:
            robot_position = self.get_robot_position()
            if robot_position is None:
                self.get_logger().warn("Robot position could not be determined.")
                return None
            robot_x, robot_y, _ = robot_position
            self.tree_map = TreeNode(robot_x, robot_y, 1)
        



        height, width = grid_data.shape
        active_area_radius = 10  # Define a small search window around the robot
        
        # Find the robot's position in the grid
         # Assuming robot starts at the center
        all_centroids=[]
        TreeNode.bfs_find_leaf_nodes(self.tree_map,naive_active_area)
        
        # Define search boundaries
        def naive_active_area(node):
            min_x = max(0, node.x - active_area_radius)
            max_x = min(width, node.x + active_area_radius)
            min_y = max(0, node.y - active_area_radius)
            max_y = min(height, node.y + active_area_radius)
            frontiers = []
            ...
            for y in range(min_y, max_y):
                for x in range(min_x, max_x):
                    if grid_data[y, x] == -1:  # Unknown cell
                        ...
                        if any(0 <= ny < height and 0 <= nx < width and grid_data[ny, nx] == 0 for ny, nx in neighbors):
            ...
            all_centroids.append(centroids_with_sizes)

            for centroid in centroids_with_sizes:
                node.add_child(centroid[0], centroid[1], centroid[2])
